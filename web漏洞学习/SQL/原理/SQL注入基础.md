...
| 函数 / 变量          | 说明                                                         |
| -------------------- | ------------------------------------------------------------ |
| **`current_user()`** | 返回服务器验证你身份时匹配的账户（如 `'root'@'%'`）。这决定了你的实际权限。 |
| **`session_user()`** | `user()` 的别名，返回你连接时提供的用户名和主机名。          |
| **`system_user()`**  | 同样是 `user()` 的别名。                                     |
| **`schema()`**       | `database()` 的别名，返回当前选中的数据库名。                |

| **函数 / 变量**                 | **说明**                                                     |
| ------------------------------- | ------------------------------------------------------------ |
| **`@@version_compile_os`**      | **非常有用**。返回操作系统类型（如 `Linux`, `Win64`）。这决定了你能否进行后续的系统级操作（如加载 `.so` 或 `.dll` 插件）。 |
| **`@@version_compile_machine`** | 返回编译时的架构（如 `x86_64`）。                            |
| **`@@version_comment`**         | 返回版本的备注信息（例如是 `MySQL Community Server` 还是 `Percona`）。 |

| **变量**                 | **说明**                                                     |
| ------------------------ | ------------------------------------------------------------ |
| **`@@datadir`**          | 数据库文件存放的绝对路径。知道这个路径通常意味着你知道了配置文件的位置。 |
| **`@@basedir`**          | MySQL 的安装路径。                                           |
| **`@@plugin_dir`**       | 插件目录。如果你想通过 UDF (User Defined Function) 提权，必须知道这个路径来上传你的共享库文件。 |
| **`@@tmpdir`**           | 临时文件目录。                                               |
| **`@@secure_file_priv`** | **极其关键**。它决定了 MySQL 是否允许执行文件的导入导出操作。如果是 `NULL`，则禁止一切读写。 |
