...
* 概述：Session反序列化是利用PHP session的不同序列化和反序列化方式之间的差异，提供了一个稳定的POP链入口
* PHP Session配置相关：
...
* PHP Session文件的存储机制：
* PHP Session反序列化机制：机制与上面的session.serialize_handler= php配置相关，这里有三种，php、php_binary、php_serialize(php>=5.5.4)
  * 
