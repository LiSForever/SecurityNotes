...
* 验证码登录
  ...
  * 使用其他手机号的验证码 ×
  * **验证码爆破**
    * 有两个用到验证码的接口，第二个才获取cookie，能否对第一个进行爆破 √
      * 第一个接口
        * 不验证sign，验证code和验证码，且无限制
        * 但要注意code的更新，$('#HiddenCodeLogin').val()，搞清其值的来源
    * 能否对第二个进行爆破
      * 第二个可能也可以爆破，但是需要先把sign算出来
      * 还有verifycode也是只能使用一次，但是只有每次发送验证码的时候才会变化
    * 是否存在会话固定 ×
  * 短信纵向轰炸
    * 绕验证码爆破
      * 目前是有两个数据包决定了验证码的发送，第一个发送图像验证码信息，获取一个token给第二个决定服务器发送短信验证码的数据包使用
      * 每个号码的每日发送存在上限，纵向爆破效果有限
    * 并发爆破 ×
  * 其他问题和发现：
    * 多个验证码  多个未使用的验证码相同 
      * 能否通过多次发送短信验证码延长同一个验证码的存活时间  ×
    * 验证码反复使用    ×
...
* <u>note</u> 发现一个账号相隔一周左右登录，其cookie的核心部分相同
  ...
  * 退出登录是否有效？ 退出登录无效，cookie在退出登录后扔可使用 √
